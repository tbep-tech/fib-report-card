...
anlz_enteromatrix <- function (fibdata, yrrng = NULL, stas = NULL, lagyr = 3) 
{
    ...
    dat <- fibdata %>% dplyr::filter(station %in% stas) %>% 
        ...
        dplyr::mutate(sumgt130 = stats::filter(sumgt130, rep(1, lagyr), sides = 1, method = "convolution"), 
                      station_tot = stats::filter(station_tot,  rep(1, lagyr), sides = 1, method = "convolution"), 
                      .by = "station") %>% 
        dplyr::mutate(exceed_10_130_prob = pbinom(sumgt130 - 1, station_tot, 0.1, lower.tail = FALSE), 
                      exceed_30_130_prob = pbinom(sumgt130 - 1, station_tot, 0.3, lower.tail = FALSE), 
                      exceed_50_130_prob = pbinom(sumgt130 - 1, station_tot, 0.5, lower.tail = FALSE), 
                      exceed_75_130_prob = pbinom(sumgt130 - 1, station_tot, 0.75, lower.tail = FALSE))
    dat$MWQA <- NA
    dat$MWQA[dat$exceed_10_130_prob >= 0.1] <- "A"
    dat$MWQA[dat$exceed_10_130_prob < 0.1 & dat$exceed_30_130_prob >= 
                 0.1] <- "B"
    dat$MWQA[dat$exceed_30_130_prob < 0.1 & dat$exceed_50_130_prob >= 
                 0.1] <- "C"
    dat$MWQA[dat$exceed_50_130_prob < 0.1 & dat$exceed_75_130_prob >= 
                 0.1] <- "D"
    dat$MWQA[dat$exceed_75_130_prob < 0.1] <- "E"
